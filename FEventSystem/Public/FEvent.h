...
#include"Reference.h"
class FEvent: public FReference
{
public:
   
};

class FGUIEvent:public FEvent
{

public :
	enum class MouseBtnMask: __int8
	{
		LEFT_MOUSE_BUTTON = 1 << 0,
		MIDDLE_MOUSE_BUTTON = 1 << 1,
		RIGHT_MOUSE_BUTTON = 1 << 2
	};
    enum class EventType: __int32 {
        NONE = 0,
        PUSH = 1 << 0,
        RELEASE = 1 << 1,
        DOUBLECLICK = 1 << 2,
    ...
    public:
        FGUIEvent(): _eventType(EventType::NONE),
            _windowX(0),
            _windowY(0),
            _windowWidth(1280),
            _windowHeight(1024),
