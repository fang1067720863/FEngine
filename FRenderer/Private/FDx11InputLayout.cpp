...
const D3D11_INPUT_ELEMENT_DESC VertexInput[1] = {
   { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 }
};
void FDx11VertexInputLayout::PrepareDxResource(FDx11GpuProgram* program)
{
    //inputLayoutDescs.data(), mElementNum,
    //HR(device.GetDevice()->CreateInputLayout(VertexInput, ARRAYSIZE(VertexInput), program->GetD3DBlob()->GetBufferPointer(), program->GetD3DBlob()->GetBufferSize(), inputLayout.GetAddressOf()));
}